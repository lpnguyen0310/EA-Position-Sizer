...
void AutoSetTakeProfitByRR(double entry, double stoploss, double rr)
{
    if(entry == 0 || stoploss == 0 || rr <= 0) return; // Không set TP nếu thiếu dữ liệu

    double tp = 0;
    if (sets.TradeDirection == Long) // BUY
        tp = NormalizeDouble(entry + rr * (entry - stoploss), _Digits);
    else // SELL
        tp = NormalizeDouble(entry - rr * (stoploss - entry), _Digits);

    sets.TakeProfitLevel = tp;
    ObjectSetDouble(ChartID(), ObjectPrefix + "TakeProfitLine", OBJPROP_PRICE, tp);
}
...
void CPositionSizeCalculator::OnClickBtnTrade()
{
	...
	if (sets.EntryLevel > 0 && sets.StopLossLevel > 0)
        AutoSetTakeProfitByRR(sets.EntryLevel, sets.StopLossLevel, RR1);
}
...
//!!!!    
    ...
    if (ShowLineLabels)
    {
	   ...
	   // Label trên line: chỉ hiện USD (trên)
		DrawLineLabel(
			ObjectPrefix + "StopLossLabel",
			FormatDouble(DoubleToString(OutputRiskMoney, AccountCurrencyDigits)) + " " + AccountCurrency,
			tStopLossLevel,
			sl_label_font_color
		);
		if (sets.ShowLines)
		...
		if (ShowAdditionalSLLabel)
		{
			...
			if (WarningSL == "")
			{
				// Label dưới line: chỉ hiện số point đã chia 100
				label_text = DoubleToString(MathAbs(tStopLossLevel - tEntryLevel) / _Point / 100.0, 2);
			}
			else label_text = WarningSL;
			DrawLineLabel(
				ObjectPrefix + "SLAdditionalLabel",
				label_text,
				tStopLossLevel,
				sl_label_font_color,
				true // <-- label phụ (dưới line)
			);
			if (sets.ShowLines)
				ObjectSetInteger(ChartID(), ObjectPrefix + "SLAdditionalLabel", OBJPROP_TIMEFRAMES, OBJ_ALL_PERIODS);
			else
		...
		if (tTakeProfitLevel > 0)
		{
			...
			if (ShowAdditionalTPLabel)
			{
				...
				{
					// Bỏ phần % phía trước, chỉ giữ USD, Currency, RR, Lot nếu có
					label_text = FormatDouble(DoubleToString(MainOutputReward, AccountCurrencyDigits)) + " " + AccountCurrency + " " + MainOutputRR + "R";
					if (sets.TakeProfitsNumber > 1)
						label_text = FormatDouble(DoubleToString(ArrayPositionSize[0], LotStep_digits), LotStep_digits) + " Lots " + label_text;
				}
				else
				{
		...
		for (int i = 1; i < sets.TakeProfitsNumber; i++)
		{
			...
			if (ShowAdditionalTPLabel)
			{
				...
				{
					// Bỏ phần % phía trước, chỉ giữ USD, Currency, RR, Lot nếu có
					label_text = FormatDouble(DoubleToString(AdditionalOutputReward[i - 1], AccountCurrencyDigits)) + " " + AccountCurrency + " " + AdditionalOutputRR[i - 1] + "R";
					label_text = FormatDouble(DoubleToString(ArrayPositionSize[i], LotStep_digits), LotStep_digits) + " Lots " + label_text;
				}
				else
				{
